...
<script type="text/javascript">
  ...
  $("#emp_save_btn").click(function(){
    ...
    alert(back_varify());
    // alert(res);
    //1、后端校验  $(this).attr("ajax-va")=="error"
    if(!back_varify()){
  ...
  /**
   * 前端校验 检测用户名邮箱格式
   * 后端校验 检测提交的格式 以及邮箱是否已注册
   */


  //校验邮箱是否可用 ===================================================================================================================================
  function back_varify(){
    //发送ajax请求校验邮箱是否可用
    ...
    $.ajax({
      ...
      success:function(result){
        ...
        console.log(result);
        if(result.code==100){
          show_validate_msg("#email_add_input","success","该邮箱尚未注册back_varify");
  ...
  function validate_add_form1() {
    ...
    if (!regName.test(empName)) {
      ...
      show_validate_msg("#empName_add_input", "error", "用户名可以是2-5位中文或者6-16位英文和数字的组合validate_add_form");
      return false;
    } else {
  ...
  function validate_add_form2(){
    ...
    if(!regEmail.test(email)){
      //alert("邮箱格式不正确");
      //应该清空这个元素之前的样式
      show_validate_msg("#email_add_input", "error", "邮箱格式不正确validate_add_form2");
